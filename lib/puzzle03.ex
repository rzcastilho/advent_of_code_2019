defmodule Puzzle03 do
  ...
  def calc_steps(wire_path, %{x: x, y: y}) do
    wire_path
    |> Enum.reduce_while(%{last: %{x: 0, y: 0}, total: 0},
         fn
           %{x: ^x, y: ^y} = coordinate, acc ->
             {
               :halt,
               %{acc|last: coordinate,total: acc.total + abs(acc.last.x - coordinate.x) + abs(acc.last.y - coordinate.y)}
             }
           coordinate, acc ->
             {
               :cont,
  ...
  def distance(%{x: x, y: y}), do: abs(x) + abs(y)

  def check_crosses([first_wire, second_wire]) do
    diff = first_wire -- second_wire
    first_wire -- diff
  end

  def process_coordinate("R" <> shif, [%{x: x} = coord|_] = acc) do
    (
      x+String.to_integer(shif)..x+1
      |> Enum.map(&(%{coord|x: &1}))
    ) ++ acc
  end

  def process_coordinate("L" <> shif, [%{x: x} = coord|_] = acc) do
    (
      x-String.to_integer(shif)..x-1
      |> Enum.map(&(%{coord|x: &1}))
    ) ++ acc
  ...
  def process_coordinate("U" <> shif, [%{y: y} = coord|_] = acc) do
    (
    y+String.to_integer(shif)..y+1
      |> Enum.map(&(%{coord|y: &1}))
    ) ++ acc
  end

  def process_coordinate("D" <> shif, [%{y: y} = coord|_] = acc) do
    (
    y-String.to_integer(shif)..y-1
      |> Enum.map(&(%{coord|y: &1}))
    ) ++ acc
  ...
  def process_wire_path(coordinates) do
    coordinates
    |> Enum.reduce([%{x: 0, y: 0}], &process_coordinate/2)
    |> Enum.reverse()
  end

  def wire(coordinates) do
    coordinates
    |> String.split(",")
  end
  ...
  def part2() do
    ...
    |> Enum.map(fn coordinate -> Enum.map(wire_paths, &(calc_steps(&1, coordinate))) end)
    |> Enum.map(fn [%{total: path1}, %{total: path2}] -> path1 + path2 end)
    |> Enum.min()
  end
